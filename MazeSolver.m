function[Policy,Value]= MazeSolver(nStates ,nActions,R,T)
width = nStates;
height = nStates;
...
% T = zeros(num_states, num_states, num_actions);
    ...
    for ii =1:nrow
        for jj=1:ncol
          ...
          if( ii==1 )                % s is ON the top row ... this action does not change our position 
            v_tmp = v_tmp + T(ii,jj,1)*( -1.5 + gamma*V(ii,jj,itr) );
            Vn(ii,jj,itr)=v_tmp;
            
          elseif( ii==2 && jj==1 )   % s is NOT on the top row but will step into a terminal state (reward is zero)

            v_tmp = v_tmp + T(ii-1,jj,1)*( R(ii-1,jj) + gamma*V(ii-1,jj,itr) ); 
            Vn(ii,jj,itr)=v_tmp;
